"""
Speech Tapper - Audio analysis for head movement synchronization.
Based on Pollen Robotics reference implementation.

Analyzes audio amplitude to generate natural head sway movements.
"""
...
# Tunables - adjusted for grumpy personality (slightly more aggressive movements)
SR = 16_000  # Sample rate
FRAME_MS = 20
HOP_MS = 10

SWAY_MASTER = 1.8  # Slightly more pronounced than reference (1.5)
SENS_DB_OFFSET = +4.0
VAD_DB_ON = -35.0
VAD_DB_OFF = -45.0
VAD_ATTACK_MS = 40
...
# Oscillator frequencies and amplitudes for head sway
SWAY_F_PITCH = 2.2
SWAY_A_PITCH_DEG = 5.0  # Slightly more expressive
SWAY_F_YAW = 0.6
SWAY_A_YAW_DEG = 8.0  # More side-to-side for attitude
SWAY_F_ROLL = 1.3
SWAY_A_ROLL_DEG = 2.5
...
def _to_float32_mono(x: NDArray[Any]) -> NDArray[np.float32]:
    ...
    # If 2D, decide which axis is channels
    if a.ndim == 2:
...
class SwayRollRT:
    """
    Real-time audio to head sway converter.

    Feed audio chunks and get back movement parameters synchronized to speech.

    Usage:
        rt = SwayRollRT()
        movements = rt.feed(pcm_data, sample_rate)
        for m in movements:
            robot.set_head_offset(m['pitch_rad'], m['yaw_rad'], m['roll_rad'])
    """

    def __init__(self, rng_seed: int = 7):
        """Initialize state with random phase offsets for natural variation."""
        ...
        # Random phase offsets for each oscillator
    ...
    def reset(self) -> None:
        """Reset state but keep initial phases."""
        self.samples.clear()
    ...
    def feed(self, pcm: NDArray[Any], sr: int | None) -> List[Dict[str, float]]:
        """
        Process audio chunk and return sway parameters.

        Args:
            pcm: Audio data as numpy array (int16 or float32)
            sr: Sample rate of the audio (None assumes SR)

        Returns:
            List of movement dicts, one per HOP_MS interval
        """
        ...
        while self.carry.size >= HOP:
            ...
            # Keep sliding window for VAD/envelope
            ...
            # Generate oscillating movements
            pitch = (
